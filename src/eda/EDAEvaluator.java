 ...
 * @see Evaluation
 */
public class EDAEvaluator {

    private Instances train_data, test_data;
    private int n_folds;

    // initializes the demo
    public EDAEvaluator(int n_folds, Instances train_data, Instances test_data) throws Exception {
...
//        data.stratify(n_folds);
        ...
        for (int i = 0; i < n_folds; i++) {
            ...
            for(int j = 0; j < n_individuals; j++) {
                EDAEnsemble edaEnsemble = new EDAEnsemble();
                edaEnsemble.setOptions(new String[]{
                ...
                edaEnsemble.buildClassifier(local_train);
                trainEval.evaluateModel(edaEnsemble, local_val);
                trainEvaluations[j] += getUnweightedAreaUnderROC(trainEval);

                if((i == n_folds - 1) && (test_data != null)) {
                    edaEnsemble.buildClassifier(train_data);

                    Evaluation testEval = new Evaluation(test_data);
                    testEval.evaluateModel(edaEnsemble, test_data);
    ...
    public double[][] broadcastEvaluationBack(int[] seeds,
        ...
        for(int i = 0; i < seeds.length; i++) {
            ...
            trainEval.crossValidateModel(edaEnsemble, train_data, n_folds, random);  // formerly filtered
    ...
    public static void main(String[] args) throws Exception {
        ...
        EDAEvaluator evaluator = new EDAEvaluator(5, train_data, test_data);
        int seed = 0;
