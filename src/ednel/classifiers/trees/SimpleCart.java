...
package ednel.classifiers.trees;
...
public class SimpleCart extends RandomizableClassifier implements
        AdditionalMeasureProducer, TechnicalInformationHandler, Drawable {
...
//        ArrayList<Integer> new_levels = new ArrayList<>(levels.size());
//        for(int i = 0; i < lines.length; i++) {
//            if(lines[i].contains(":")) {
//                String[] partitioned = lines[i].split(":");
//                new_lines.add(partitioned[0].trim());
//                new_levels.add(levels.get(i));
//                new_lines.add(partitioned[1].trim());
//                new_levels.add(levels.get(i) + 1);
//
//            } else {
//                new_lines.add(lines[i]);
//                new_levels.add(levels.get(i));
...
//        }
        ...
        StringBuffer text = new StringBuffer("digraph SimpleCART_Tree {\n");
        for(int i = 0; i < new_lines.size(); i++) {
            int parent_index = i - 1;
            boolean foundParent = false;
            while(parent_index >= 0) {
                if(levels.get(parent_index) == (levels.get(i) - 1)) {
                    foundParent = true;
                    break;
                }
                parent_index--;
            ...
            int coparent_index = parent_index - 1;
            boolean foundCoparent = false;
            while(coparent_index >= 0) {
                if(!levels.get(coparent_index).equals(levels.get(parent_index))) {
                    coparent_index--;
                } else {
                    foundCoparent = true;
                    break;
            ...
            if(!foundCoparent) {
                coparent_index = parent_index;
            }

            int sibling_index = i - 1;
            boolean foundSibling = false;
            while(sibling_index >= 0) {
                if(!levels.get(sibling_index).equals(levels.get(i))) {
                    sibling_index--;
                } else {
                    foundSibling = true;
                    break;
            ...
            String[] this_node_parted = new_lines.get(i).split("<=|>=|>|<|!=|=");
            String[] sibling_node_parted = new_lines.get(sibling_index).split("<=|>=|>|<|!=|=");
            boolean link = true;
            if(this_node_parted.length == 1) {  // leaf node
                text.append(String.format("N%d [label=\"%s\" shape=box style=filled]\n", i, this_node_parted[0].replaceAll("\\(", " (")));
            } else if(foundSibling && this_node_parted[0].equals(sibling_node_parted[0])) {
//                to_report_index = sibling_index;
                link = false;
//                text.append(String.format("N%d [label=\"%s\"]\n", to_report_index, sibling_node_parted[0].trim()));
            } else {
                text.append(String.format("N%d [label=\"%s\"]\n", i, this_node_parted[0].trim()));
            }

            // if found parent
            if(foundParent && link) {
                String[] parent_parted = new_lines.get(parent_index).split("<=|>=|>|<|!=|=");
                String[] coparent_parted = new_lines.get(coparent_index).split("<=|>=|>|<|!=|=");
                String parent_symbol = new_lines.get(parent_index).substring(
                    new_lines.get(parent_index).indexOf(parent_parted[0]) + parent_parted[0].length(),
                    new_lines.get(parent_index).indexOf(parent_parted[1])
                );
                int parent_link_index = (foundCoparent && parent_parted[0].equals(coparent_parted[0]))? coparent_index : parent_index;
                text.append(
                    String.format("N%d->N%d [label=\"%s %s\"]\n",
                        parent_link_index,
                        i, parent_symbol, parent_parted[1]
                    )
                );
            }
        }
